...
terminator = {
	serverAddress: "survival.mc.jpdld.com",
	inGameDefaultPass: "hellomraccountowner",
	bots: {},
		...
		return new Promise(function(resolve, reject) { // resolve 1 = successfully joined game, resolve 0 = timed out joining game
								...
								var execute = function() {
									if (target) {
										...
										var end = function() { // function to end intervals and reset intervals when done killing NPC:
											terminator.bots[_id].temp.targetI.targeting = false;
											terminator.bots[_id].temp.attackStatus.target = "n/a";
											clearInterval(terminator.bots[_id].temp.attackint);
											clearInterval(terminator.bots[_id].temp.checkcompleteint);
											clearInterval(terminator.bots[_id].temp.navigationint);
										...
										var gototarget = function() { // use mineflayer-navigate to reach the target:
											try {
												terminator.bots[_id].mf.navigate.to(target.position);
											} catch (err) {
										...
										terminator.bots[_id].temp.navigationint2 = setInterval(function() {
												if (terminator.bots[_id].mf.players[username].entity) { // if bot not dead
													if (target) { // if target entity exists: (they're not dead/out of range)
														if ((Math.abs(Math.floor(target.position.x) - Math.floor(terminator.bots[_id].mf.players[username].entity.position.x)) >= 6) == false && (Math.abs(Math.floor(target.position.z) - Math.floor(terminator.bots[_id].mf.players[username].entity.position.z)) >= 6) == false) {
															if (usenavigationplugin) {
																...
																clearInterval(terminator.bots[_id].temp.updatenearestentint);
																terminator.bots[_id].temp.watchentity = target;
																terminator.bots[_id].temp.watch = true;
															} else {
																if (!terminator.bots[_id].mf.navigate.currentlybusy) {
																	terminator.bots[_id].mf.setControlState("forward", true);
										...
										terminator.bots[_id].temp.attackint = setInterval(function() {
												if (target) {
													terminator.bots[_id].mf.attack(target);
													terminator.bots[_id].temp.functions.equipWeapon();
												}
										}, 800);
										// interval to manage weapons and attacking the target --
			...
			terminator.bots[_id].mf.on('spawn', function() { // when bot spawns
				...
				if (!terminator.bots[_id].temp.isReady) {
					...
					if (serverAccount) {
						terminator.bots[_id].mf.chat(`/register ${terminator.inGameDefaultPass} ${terminator.inGameDefaultPass}`);
						terminator.bots[_id].mf.chat(`/login ${terminator.inGameDefaultPass}`);
					}
				}
			});
			...
			terminator.bots[_id].mf.on("death", function() { // when bot dies:
				...
				terminator.bots[_id].temp.equipped.armor = {
					helmet: "n/a",
					chestplate: "n/a",
					leggings: "n/a",
					boots: "n/a"
